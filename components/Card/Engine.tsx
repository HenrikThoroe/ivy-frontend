...
export default function EngineCard<T extends Variant>(
  ...
  if (props.variant === 'template') {
    return (
      <Link href="/engines/upload">
        <div className="group flex h-[177px] w-[386px] cursor-pointer flex-row items-center justify-center gap-[20px] rounded-[18px] border-[1px] border-dashed border-on-primary hover:border-action-primary-active">
  ...
  return (
    <Link href={`/engines/${props.name}`}>
      <div className="flex h-[177px] w-[386px] cursor-pointer flex-col items-start justify-center gap-[1.25rem] rounded-[1.125rem] border-2 border-solid border-on-primary px-[1.25rem] py-[1.875rem] hover:border-action-primary-active">
        <h3 className="text-[1.5rem] font-semibold text-on-primary">{props.name}</h3>
        <div className="flex flex-col gap-[0.625rem]">
          <div className="flex flex-row items-center justify-start text-on-primary">
